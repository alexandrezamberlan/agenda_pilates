...
class UsuarioRegisterForm(forms.ModelForm):
    ...
    tipo = forms.ChoiceField(label='Tipo *',choices=TIPOS_USUARIOS, help_text='Este processo cadastra somente pacientes', required=True)
    nome = forms.CharField(label='Nome completo *', help_text='* Campos obrigatórios',required=True)
    sexo = forms.ChoiceField(label='Sexo *', choices=SEXO, help_text='* Campos obrigatórios',required=True)
    data_nascimento = forms.DateField(label='Data de nascimento *', help_text='* Campos obrigatórios',required=True)
    ...
    class Meta:
        model = Usuario
        fields = ['tipo','nome','sexo', 'data_nascimento', 'email', 'celular', 'cpf', 'password','aceita_termo']
