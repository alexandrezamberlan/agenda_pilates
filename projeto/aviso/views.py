...
class AvisoListView(LoginRequiredMixin, FisioterapeutaRequiredMixin, ListView):
    model = Aviso


class AvisoCreateView(LoginRequiredMixin, FisioterapeutaRequiredMixin, CreateView):
    model = Aviso
    ...
    def form_valid(self, form):
        ...
        if (aviso.enviado):
            ...
            elif aviso.destinatario == 'FISIOTERAPEUTA':
                destinatarios = Usuario.objects.filter(is_active=True, tipo='FISIOTERAPEUTA')
            elif aviso.destinatario == 'SECRETARIA':
                destinatarios = Usuario.objects.filter(is_active=True, tipo='SECRETARIA')
            else:
                destinatarios = Usuario.objects.filter(is_active=True, tipo='PACIENTE')
...
class AvisoUpdateView(LoginRequiredMixin, FisioterapeutaRequiredMixin, UpdateView):
    model = Aviso
...
class AvisoEnviaEmail(LoginRequiredMixin, FisioterapeutaRequiredMixin, RedirectView):
